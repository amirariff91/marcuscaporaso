...
async function readBrief(slug: string): Promise<string | null> {
  try {
    return await readFile(path.join(BRIEFS_DIR, `${slug}.md`), "utf8");
  } catch {
    return null;
  }
}
...
export async function generateMetadata({
  ...
  return {
    title: `${titleCase(slug)} — OSWA Content Brief`,
...
export default async function ContentBriefPage({
  ...
  const { slug } = await params;
  const markdown = await readBrief(slug);
