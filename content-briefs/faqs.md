# Content Brief: Weight Loss Surgery FAQs
**URL:** /faqs
**Primary keyword:** weight loss surgery faqs perth
**Search intent:** informational (research-stage patient gathering answers across the whole topic before committing to a call)
**Target word count:** 1,400–1,800 words (the FAQ list is the centrepiece and warrants depth; this is a hub page that aggregates answers from across the site, so it runs longer than the single-topic pages while staying scannable through the accordion format)

## Page purpose
This is the hub / orientation page for Obesity Surgery WA. It gathers the questions a prospective patient asks across the entire journey — eligibility, procedures, cost and funding, safety, recovery, surgeons, and life after surgery — into one place. Its job is to remove uncertainty at scale, internally link out to the deeper single-topic pages, and move the reader toward an orientation consultation. It deliberately does not try to fully answer every topic; it answers the common question and points to the dedicated page for detail.

---

## Keyword Intelligence

### Primary Keyword
- Keyword: weight loss surgery faqs perth
- AU Monthly Volume: ~10 (estimate; the exact phrase is a low-volume long-tail head term)
- Keyword Difficulty: ~5/100 (estimate; FAQ-hub intent is weakly contested)
- CPC (AU): ~$1.3 (estimate, in line with the broader weight-loss-surgery-perth cluster)

> Note: the exact primary phrase is low volume. Do NOT write to a single exact-match phrase. This is a hub page — its SEO value is FAQ-schema coverage of the People Also Ask cluster and internal-linking equity to the money pages, not raw head-term volume.

### Secondary Keywords (target naturally in body copy)
All metrics below are reasonable estimates aligned with the sibling briefs in this set; treat as directional, not measured.

| Keyword | AU Vol/mo (est.) | KD (est.) |
|---|---|---|
| bariatric surgery questions | 30 | 5 |
| gastric sleeve faqs | 20 | 5 |
| is weight loss surgery safe | 100 | 10 |
| weight loss surgery recovery time | 80 | 10 |
| does medicare cover weight loss surgery | 30 | 1 |
| how much does weight loss surgery cost | 40 | 0 |
| weight loss surgery perth | 250 | 45 |
| am i eligible for weight loss surgery | 10 | 0 |

### Semantic Terms to Include
Comma-separated list of LSI/semantic terms the writer must naturally weave into the copy. These are not keyword-stuff targets — they are terms that signal topical depth to Google.

bariatric surgery, gastric sleeve, sleeve gastrectomy, gastric bypass, mini gastric bypass, adjustable gastric band, revision surgery, weight-loss medication, BMI, comorbidities, type 2 diabetes, obstructive sleep apnoea, FRACS, Royal Australasian College of Surgeons, laparoscopic, keyhole surgery, St John of God Murdoch, Subiaco, Medicare item number, private health cover, 12-month waiting period, superannuation early release, out-of-pocket gap, GP referral, dietitian, excess weight loss, recovery, loose skin, weight regain

### SERP Landscape (AU)
A dedicated "weight loss surgery FAQs" page sits in informational territory where the competition is mixed: high-authority reference explainers (Healthdirect, Mayo Clinic) that answer individual questions, and a long tail of low-DR Perth/Australian bariatric clinic FAQ pages that bundle questions into accordions. No single Perth competitor owns a comprehensive, schema-rich FAQ hub that spans eligibility, every procedure, funding, safety, recovery and after-surgery life in one place. Google surfaces a strong People Also Ask cluster across this topic — "Is weight loss surgery safe?", "How much does weight loss surgery cost in Perth?", "Is weight loss surgery covered by Medicare in Australia?", "How much do I need to weigh to qualify?", "How long is recovery after gastric sleeve?" — and these are the snippets a well-structured FAQ-schema page is built to win. Because this page aggregates intent the single-topic pages already serve, it is best treated as a PAA-capture and internal-linking asset rather than a standalone ranking play for one head term.

Top 5 reference / competing page types (illustrative):
| # | Type | Example framing |
|---|---|---|
| 1 | Government reference | Healthdirect — Guide to bariatric surgery |
| 2 | Clinic FAQ accordion | Australian bariatric clinic "Frequently Asked Questions" pages |
| 3 | Authority explainer | Mayo Clinic — Weight-loss surgery Q&A |
| 4 | Surgeon homepage FAQ block | Perth bariatric surgeon FAQ sections |
| 5 | PAA / featured snippet | Google People Also Ask cluster |

### Content Gap Opportunity
1. **No Perth FAQ hub.** No ranking Perth competitor consolidates the full journey into one schema-eligible FAQ page that names local hospitals (SJOG Murdoch/Subiaco), local consulting rooms (Booragoon, Mandurah) and FRACS credentials. Owning the comprehensive local FAQ hub is a clean opening.
2. **Procedure breadth.** Most clinic FAQ pages are single-surgeon and answer only the procedures that surgeon performs. OSWA can answer across sleeve, bypass, mini bypass, band, revision and non-surgical/medication options in one place.
3. **Funding clarity.** The PAA shows persistent demand for cost, Medicare and super questions, which clinic FAQ pages cover thinly. Answering these factually (Medicare items, private cover gap, super on medical grounds) without dollar figures fills the gap and feeds the money pages.
4. **Internal-linking equity.** As the hub, this page can pass equity to am-i-eligible, gastric-sleeve-cost-perth, medicare-private-health-cover, surgeons and weight-loss-surgery-perth — a structural win the competing single pages cannot replicate.
5. **PAA real estate.** Matching FAQ schema to the live PAA questions ("is it safe", "recovery time", "does Medicare cover it") is the most direct route to featured-snippet placement for terms the broad clinic homepages bury.

### Recommended Word Count
1,400–1,800 words (the FAQ list is the centrepiece; a hub page aggregating the full journey runs longer than the single-topic pages while staying scannable via the accordion).

---

## Section guidance

This page uses the shared OswContentPage template, so every section field must be populated. For this hub page the non-FAQ sections take an overview/orientation framing and defer detail to the dedicated pages; the FAQ list is the star and should be the richest, most carefully written section.

### Hero
- **Headline direction:** Plain and literal — name the page for what it is. The reader searched for answers; the headline should confirm they have found the right place.
- **Subheadline:** Warm and orienting. Frame the page as "answers to the most common questions about weight loss surgery in Perth", gathered in one place. Signal the breadth (eligibility, procedures, cost and Medicare, safety, recovery, life after surgery) and reassure there is no pressure to decide anything today. Two to three sentences.

### Stats bar (4 stats)
Orientation anchors, not marketing claims:
1. **Procedures offered / 6** — sleeve, bypass, mini bypass, band, revision and medication/non-surgical options
2. **Hospitals / 2** — St John of God Murdoch and Subiaco
3. **Care team / 10+** — surgeons, physicians and dietitians
4. **Clinic locations / Perth + Mandurah** — Booragoon and Mandurah consulting rooms

### Your questions about weight loss surgery in Perth, answered (3 paragraphs)
- **Paragraph 1 — what this page is:** Explain that most people arrive with the same questions, and this page brings them together so the reader can get straight answers before calling. Frame it as orientation, not the final word — the answer that matters is the surgeon's, about the reader's own situation.
- **Paragraph 2 — the options at OSWA:** Define weight loss / bariatric surgery in plain terms (procedures that change stomach size or function to help you eat less and feel full sooner). Name the full menu: gastric sleeve, gastric bypass, mini gastric bypass, gastric band, revision surgery, plus weight-loss medication / non-surgical options. State that surgery is one tool among several, not the only road.
- **Paragraph 3 — the team:** Whichever question brought them here, they are stepping into a team — surgeons supported by physicians and dietitians who stay involved before and long after any procedure. Point the reader to the full FAQ list further down.

### Comparison note (inline callout)
Practical guidance for the reader wondering which procedure suits them: this is not a website decision. Sleeve, bypass and band each suit different bodies, goals and health histories, and the right match is worked out with the surgeon at consultation. Defer to surgeon assessment.

### Eligibility / Who is suitable?
Standard BMI-threshold-as-starting-point framing:
- General guide: BMI 35+ typically suitable for a surgical assessment; BMI 30–34 may qualify when weight-related conditions are present. Starting points, not a final ruling; the surgeon confirms.
- BMI is part of the picture, not the whole — whole-person assessment (conditions, history, goals).
- Weight-related conditions that strengthen the case: type 2 diabetes, high blood pressure, obstructive sleep apnoea, severe reflux, PCOS, fatty liver disease.
- A history of diets that did not hold is expected, not a mark against the reader.
- Mental health history is part of the assessment and never on its own a reason to be turned away.
- GP referral is the usual first step and gives access to Medicare rebates; direct contact also possible.

### How it works (6 steps)
The established OSWA journey. Two to three sentences each:
1. **GP Referral** — referral to OSWA gives the higher referred Medicare rebate and lets the team review history; direct contact also possible
2. **Orientation Consultation** — meet the surgeon, talk through history, options explained in plain language, no pressure to decide on the day
3. **Pre-operative Assessment** — physicians and dietitians assess health, screen for reflux/sleep apnoea, prepare for surgery safely
4. **Surgery** — FRACS surgeon performs the keyhole (laparoscopic) procedure at SJOG Murdoch or Subiaco
5. **Hospital Recovery** — recovery in hospital, usually a few nights, with nursing and dietitian support before discharge
6. **Ongoing Support** — dietitians and physicians stay involved long term to protect results

### Why patients choose Obesity Surgery WA (benefits, 5 bullets)
Concrete and factual, no superlatives:
- FRACS-credentialled lead surgeon, Notre Dame lecturer, personal bariatric surgery experience
- Two experienced surgeons so procedure matching is possible, not one-size-fits-all
- Operating privileges at SJOG Murdoch and Subiaco (accredited hospitals)
- In-house physicians and dietitians supporting pre- and post-operative care
- Two clinic locations (Perth and Mandurah) for accessibility

Avoid: "best", "leading", "world-class", and any language implying superior outcomes.

### Being honest about what surgery can and cannot do (risks)
- **Opening paragraph:** No surgeon can promise a specific result; be cautious of anyone who does. Surgery is a powerful tool that works alongside the patient's effort, not instead of it. Note honestly that the answer to many FAQ questions is "it depends on you, your starting health and the procedure".
- **Risk bullets (5 items):**
  - All surgery carries risk: bleeding, infection, blood clots, leaks, anaesthetic reactions
  - Bariatric procedures require lifelong changes to eating and ongoing follow-up
  - Some procedures require long-term vitamin and mineral supplementation
  - Loose skin after significant weight loss varies between individuals; may be addressable later
  - Long-term success depends on sustained dietary and lifestyle change; weight can return without it

### Cost paragraph
No dollar figures. Cover: cost depends on procedure, private cover and individual circumstances; the total is several separate fees (surgeon, anaesthetist, hospital); most procedures attract a Medicare item number; Medicare contributes toward surgical/anaesthetist/consultation fees but not the hospital stay or theatre (that is private cover's role); appropriate private hospital cover usually means a gap rather than the full fee; super early release on medical grounds and third-party payment plans are options some use; a clear, itemised written estimate is provided before any commitment.

### Surgeon profiles (2 profiles)
Copy from the surgeons page — full credentials in every reference.

**Dr Harsha Chandraratna — MBBS, FRACS, Bariatric Surgeon**
Lead surgeon and founder, FRACS, Notre Dame lecturer, operates at SJOG Murdoch and Subiaco, personal experience of bariatric surgery shaping his patient-centred approach.

**Dr Stephen Watson — MBBS, FRACS, Bariatric Surgeon**
Experienced bariatric surgeon performing sleeve, bypass and revisional procedures; thorough, unhurried, procedure-matching approach.

### Outcomes note
Clinical averages with caveat. Published averages typically show 60–80% of excess weight lost in the first one to two years depending on procedure. Lead with the caveat: averages only, not individual promises. Results vary with starting health, procedure and adherence; no specific outcome can be promised.

### Conversion CTA headline
Direction: warm, low-pressure, hub-appropriate. Invite the reader who still has an unanswered question to talk it through. In the spirit of: "Still have a question we have not answered here? Talk it through with our team, no pressure."

### FAQs (the centrepiece — 16 questions)
This is the most important section on the page. Aggregate and lightly adapt the strongest questions already written across the site so answers stay consistent and never contradict the dedicated pages. Span the full journey: eligibility, procedures, cost/Medicare/super, safety/risks, recovery, surgeons, and life after surgery. Plain language, no jargon, no hedging beyond what compliance requires.

1. Am I eligible for weight loss surgery? (BMI 35+ typically; 40+ almost always; 30–34 with conditions; whole-person assessment; surgeon confirms)
2. My BMI is around 30 — am I too low? (May qualify with a weight-related condition; type 2 diabetes can change it; otherwise medication/dietitian support; no one turned away without options)
3. What weight loss surgery procedures do you offer? (Sleeve, bypass, mini bypass, band, revision, plus medication/non-surgical; surgeon helps match)
4. What is the difference between a gastric sleeve, a gastric bypass and a gastric band? (Plain-language description of each; mini bypass as a bypass variation; right choice decided with surgeon)
5. Is weight loss surgery safe? (Well-established, keyhole/laparoscopic at accredited SJOG hospitals; but all surgery carries risk — bleeding, infection, clots, leaks, anaesthetic; risk not minimised; surgeon reviews individual profile)
6. How much does weight loss surgery cost in Perth? (No single price — separate fees; depends on procedure, cover, circumstances; Medicare item applies; gap rather than full fee with appropriate cover; written estimate)
7. Does Medicare cover weight loss surgery? (Rebate toward eligible surgical/anaesthetist/consultation fees when criteria met and GP referral provided; not the full cost; not hospital/theatre; gap typically remains)
8. Do I need private health insurance, and can I use my super? (Hospital cover including bariatric item numbers; 12-month waiting period and higher tier common; confirm item numbers with fund; super early release on medical grounds assessed by ATO/fund not the practice; payment plans sometimes used)
9. Will I see the surgeon, or someone else? (FRACS surgeon assesses and operates, not a trainee; physicians and dietitians form the wider team)
10. What qualifications do your surgeons have? (Both MBBS, FRACS; Dr Chandraratna lectures at Notre Dame and has personal bariatric surgery experience; both operate at SJOG Murdoch and Subiaco)
11. How long is the recovery, and how soon can I go back to work? (Usually a few nights in hospital; two to four weeks off work depending on procedure and job; office work often sooner; surgeon advises)
12. Can I eat normally again after surgery? (Eating changes early while healing; most return to a wide range of foods in smaller amounts; dietitian guides reintroduction; goal is a healthier relationship with food)
13. How much weight will I lose? (Averages 60–80% excess weight in one to two years depending on procedure; averages not promises; depends on starting health, procedure, adherence; results vary, no guarantee)
14. Where are your clinics, and where is surgery performed? (Booragoon — Garden City Specialist Centre, 537 Marmion Street; Mandurah — WA Cardiology Centre, 30 Minilya Pkwy; surgery at SJOG Murdoch and Subiaco)
15. What if the surgery does not work for me? (Results vary; some regain weight if habits drift; team investigates why; revision surgery sometimes an option; this is why long-term support matters)
16. Do I have to have surgery, or are there other options? (No — obesity is a medical condition not a willpower failure; medication, dietitian support and non-surgical options exist, alone or alongside surgery over time; many start with medical management; first consultation is the place to start)

---

## Tone and compliance notes
- This is a hub page: each FAQ answer should give the common answer and, where natural, defer detail to the surgeon or to the dedicated page — do not contradict any of the single-topic pages.
- Both surgeons must be identified with full credentials (MBBS, FRACS) wherever referenced.
- No outcome guarantees, no "you will lose X kg", no "guaranteed results" — weight-loss figures always framed as clinical averages with an explicit variance caveat.
- No patient testimonials or case studies — AHPRA prohibits testimonial-style claims for regulated health services.
- No "best", "leading", or "world-class" claims; all comparative claims require substantiation.
- Surgical risk information must be present and must not be minimised or buried — the safety FAQ in particular must state the real risks.
- Medicare item numbers may be mentioned factually; eligibility must always be described as clinically assessed, not assumed.
- Super early release: describe accurately as assessed by the ATO and super fund on medical grounds; do not imply it is automatic or straightforward.
- Recovery framed as "usually a few nights" in hospital — do not state a fixed "3–4 nights".
- No specific dollar figures on this page; refer to the written estimate and the dedicated cost pages.
- Voice: direct, credible, clinical — this is a practice, not a wellness brand.
